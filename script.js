...
try {
  ...
  const saved =
    localStorage.getItem("teachlyV3");
  ...
  /* Make sure older saved accounts have all properties */
  ...
  console.log("Saved data was corrupted. Resetting Teachly.");
...
function getAIAnswer(q) {
  ...
  return `
    Great question! Based on today's topic,
    think about the main idea:
    <strong>${escapeHtml(currentTopic.name)}</strong>.
    I can explain the lesson in simpler words
    if you ask!
  `;
